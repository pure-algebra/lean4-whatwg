...
| Promise census breaker (own worktree, branch `promise/census-breaker`) | `test/contracts/webidl-census.contract.md`, `test/contracts/ecma262-census.contract.md`, frozen batteries under `WhatwgTest/Audit/WebIdl/` and `WhatwgTest/Audit/Ecma262/`, plus the byte-identity battery for the two existing projections; red until the builders land |
| Promise builders and reviewer | Claimed after the breaker freezes; builders in their own worktrees on `promise/webidl-census` (generator profile refactor and Web IDL standard) and `promise/ecma262-census` (ecmarkup scanner and ES2026 standard) |
