...
| `Whatwg/Html` | the HTML content model ported from TyXML's row types under the HTML Standard's authority: `Schema` (generated by `lake exe tyxmlschema --write`: tag and attribute universes, every named content and attribute set as a constructor-dispatch membership function, element and attribute rows; data and derived instances only, landed at H2), `Content` (admission, transparency, lattice, divergence rows), `Node`, `Print`, `Syntax`, `Bridge` (declaration-free stubs) |
