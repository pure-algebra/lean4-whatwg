...
| `UnitPromise`, `SinkAnswer`, `SinkReturn` | views of `Whatwg.Ecma262.Promise.State`, `.Outcome` and `.Returned`, the canonical owners since Q3 (`E-04`..`E-06`, move), reached through the Readable abbrevs; the twelve exact view/round-trip receipts `E-07`..`E-12` are the conversion theorems and stay `rfl` | `foreignBoundary` objects, owned first-order outcomes; PROCESSWRITE, PROCESSCLOSE, ABORTSTEPS |
...
| `State`, promise/identity slot views | one post-start writer/controller projection; `promises`, `nextPromise` and `handled` are a view of the canonical `Whatwg.Ecma262.Promise.Table Unit (Boundary.Exception ε)` through `Writable.promiseTable`, with `promiseTable_eq`, `lookupPromise_bridge`, `freshPromise_bridge`, `settle_bridge`, `markHandled_bridge` and `handled_bridge` as its receipts (`E-13`..`E-15`, `E-18`..`E-21`, generalize) | `owned`; all anchors; global freshness embedding open |
