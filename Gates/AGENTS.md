...
is no generated coverage file to hand-edit. The cost is a coupling: building
this executable, and so every mode of it, needs that module to elaborate.
`--write` does not read the emit, so a census the numerator's freeze has not
caught up with is still regenerated by building `Gates` alone and running
`Gates.Census.write` under `lake env lean --run`.
